...
function drawSky(dt, clockMs) {
  ...
  if (Scenes.current === 'hub' && desktop()) {
    ...
    for (const e of ps) if (e.p.depth > 0) drawWorld(e);
    drawShip(dt, clockMs);                             /* the vessel rides nearest the glass */
...
/* ============================================================
   THE SHIP — the surveyor's vessel, seen at last.
   You were always aboard her: the porthole, the hull groans, the slow
   drift in from the dark. In orbit she holds station off the Artifact;
   choose a world and she banks and runs ahead of the flood; while you
   walk a world she is not drawn (you are IN her, looking out); return
   to orbit and she glides back to station. Reduced motion holds her at
   station, engines cold. Sprites baked once; zero per-frame allocation.
   ============================================================ */
const ship = {
  x: 0, y: 0, a: 0, drive: 0,
  mode: 'station', t: 0, seen: false,
  fx: 0, fy: 0, tx: 0, ty: 0,                          /* flight from / to */
};
let shipSpr = null, plumeSpr = null, shipLastWorld = null;
function bakeShip() {
  /* the hull: a brass-trimmed gunmetal survey sloop, nose to the right,
     one warm-lit porthole amidships (the one you look out of) */
  const c = document.createElement('canvas'); c.width = 96; c.height = 64;
  const g = c.getContext('2d');
  g.beginPath();
  g.moveTo(80, 32);
  g.bezierCurveTo(66, 20, 40, 18, 24, 26);
  g.bezierCurveTo(18, 29, 18, 35, 24, 38);
  g.bezierCurveTo(40, 46, 66, 44, 80, 32);
  g.closePath();
  const hull = g.createLinearGradient(0, 18, 0, 46);
  hull.addColorStop(0, '#3a4356'); hull.addColorStop(0.45, '#232a3a'); hull.addColorStop(1, '#12161f');
  g.fillStyle = hull; g.fill();
  g.strokeStyle = 'rgba(201,163,92,0.85)'; g.lineWidth = 1.2; g.stroke();
  g.beginPath();                                        /* dorsal fin */
  g.moveTo(38, 22); g.quadraticCurveTo(34, 12, 28, 10); g.quadraticCurveTo(30, 18, 33, 23); g.closePath();
  g.fillStyle = '#1c2230'; g.fill();
  g.strokeStyle = 'rgba(201,163,92,0.5)'; g.lineWidth = 1; g.stroke();
  g.beginPath();                                        /* keel fin */
  g.moveTo(40, 42); g.quadraticCurveTo(36, 50, 31, 52); g.quadraticCurveTo(34, 46, 36, 41); g.closePath();
  g.fill(); g.stroke();
  g.fillStyle = '#0d1119';                              /* engine housing at the tail */
  g.beginPath(); g.ellipse(22, 32, 4, 6, 0, 0, 7); g.fill();
  g.strokeStyle = 'rgba(201,163,92,0.6)'; g.stroke();
  g.strokeStyle = 'rgba(201,163,92,0.9)'; g.lineWidth = 1;  /* the porthole, brass-ringed */
  g.beginPath(); g.arc(52, 31, 3.6, 0, 7); g.stroke();
  g.fillStyle = 'rgba(255,217,160,0.95)';
  g.beginPath(); g.arc(52, 31, 2.4, 0, 7); g.fill();
  g.fillStyle = 'rgba(168,233,255,0.8)';                /* cockpit glint at the nose */
  g.beginPath(); g.arc(70, 29, 1.8, 0, 7); g.fill();
  shipSpr = c;
  /* the drive plume: one baked additive teardrop, scaled live by thrust */
  const pc = document.createElement('canvas'); pc.width = 80; pc.height = 36;
  const pg = pc.getContext('2d');
  const pgr = pg.createLinearGradient(78, 0, 0, 0);
  pgr.addColorStop(0, 'rgba(190,235,255,0.85)');
  pgr.addColorStop(0.3, 'rgba(120,200,255,0.4)');
  pgr.addColorStop(1, 'rgba(80,140,255,0)');
  pg.fillStyle = pgr;
  pg.beginPath();
  pg.moveTo(78, 18);
  pg.quadraticCurveTo(40, 2, 2, 16);
  pg.quadraticCurveTo(0, 18, 2, 20);
  pg.quadraticCurveTo(40, 34, 78, 18);
  pg.closePath(); pg.fill();
  plumeSpr = pc;
}
function shipStation() {
  /* moored INSIDE the orbit ring, just off the Artifact's glow — the
     worlds ride the ellipse; the water close to the sphere stays clear */
  return desktop()
    ? { x: ART.cx + ART.r * 1.5, y: ART.cy - ART.r * 0.18 }
    : { x: ART.cx + ART.r * 0.5, y: ART.cy - ART.r * 1.35 };
}
function drawShip(dt, clockMs) {
  if (!shipSpr) bakeShip();
  const st = shipStation();
  const t = clockMs / 1000;
  if (reduced()) {                                      /* the designed pose: engines cold */
    ship.x = st.x; ship.y = st.y; ship.a = -0.06; ship.drive = 0; ship.mode = 'station'; ship.seen = true;
  } else if (ship.mode === 'station') {
    if (!ship.seen) {                                   /* first sight: she drifts in from the dark */
      ship.x = st.x - W * 0.26; ship.y = st.y + H * 0.12; ship.a = 0.1; ship.seen = true;
    }
    const k = 1 - Math.exp(-dt / 1400);                 /* long, patient ease */
    ship.x += (st.x - ship.x) * k;
    ship.y += (st.y + Math.sin(t * 0.6) * 4 - ship.y) * k;
    ship.a += (-0.06 + Math.sin(t * 0.4 + 1) * 0.05 - ship.a) * k;
    ship.drive += (0.1 - ship.drive) * k;               /* thrusters barely breathing */
  } else if (ship.mode === 'out') {                     /* she runs ahead of the flood */
    ship.t = Math.min(1, ship.t + dt / 480);
    const p = ship.t * ship.t;
    ship.x = ship.fx + (ship.tx - ship.fx) * p;
    ship.y = ship.fy + (ship.ty - ship.fy) * p;
    ship.a = Math.atan2(ship.ty - ship.fy, ship.tx - ship.fx);
    ship.drive = 0.4 + p * 1.1;
  } else if (ship.mode === 'in') {                      /* home again: glide back to station */
    ship.t = Math.min(1, ship.t + dt / 900);
    const q = 1 - (1 - ship.t) * (1 - ship.t) * (1 - ship.t);
    ship.x = ship.fx + (st.x - ship.fx) * q;
    ship.y = ship.fy + (st.y - ship.fy) * q;
    const want = ship.t > 0.7 ? -0.06 : Math.atan2(st.y - ship.fy, st.x - ship.fx);
    ship.a += (want - ship.a) * Math.min(1, dt / 220);
    ship.drive = 1 - q * 0.9;
    if (ship.t >= 1) ship.mode = 'station';
  }
  const s = desktop() ? 0.52 : 0.4;
  ctx.save();
  ctx.translate(ship.x, ship.y);
  ctx.rotate(ship.a);
  if (ship.drive > 0.03) {                              /* plume first, additive, behind the tail */
    const flick = reduced() ? 1 : 0.82 + 0.18 * Math.sin(clockMs * 0.021);
    const pl = (18 + ship.drive * 52) * s * flick;
    const ph = (12 + ship.drive * 7) * s;
    ctx.globalCompositeOperation = 'lighter';
    ctx.globalAlpha = Math.min(1, 0.35 + ship.drive * 0.55);
    ctx.drawImage(plumeSpr, -30 * s - pl, -ph / 2, pl, ph);
    ctx.globalAlpha = 1;
    ctx.globalCompositeOperation = 'source-over';
  }
  ctx.drawImage(shipSpr, -48 * s, -32 * s, 96 * s, 64 * s);
  if (!reduced() && (clockMs % 1600) < 150) {           /* the nav light, keeping honest time */
    ctx.fillStyle = 'rgba(249,115,102,0.95)';
    ctx.beginPath(); ctx.arc(-20 * s, -23 * s, 1.6, 0, 7); ctx.fill();
  }
  ctx.restore();
}
/* she answers the same public events everything else does */
Orrery.events.addEventListener('warp', () => {
  if (reduced()) return;
  ship.mode = 'out'; ship.t = 0;
  ship.fx = ship.x; ship.fy = ship.y;
  ship.tx = warp.cx * W; ship.ty = warp.cy * H;
});
Orrery.events.addEventListener('scene', (e) => {
  const name = e.detail && e.detail.name;
  if (name && name !== 'hub') { shipLastWorld = name; return; }
  if (ship.seen && shipLastWorld && !reduced()) {       /* back in orbit: in from the world we left */
    const i = WORLDS.indexOf(bySlug[shipLastWorld]);
    const p = (desktop() && i >= 0) ? planetPos(i, Ticker.clock) : { x: W / 2, y: H * 0.45 };
    ship.mode = 'in'; ship.t = 0;
    ship.fx = p.x; ship.fy = p.y; ship.x = p.x; ship.y = p.y;
  }
});
