...
const SphereForge = (() => {
  ...
  function buildHull(rnd) {
    ...
    /* seeded panel seams + patch plates + a few scars */
    for (let x = 96; x < 830; x += 18 + rnd() * 26) {
      g.strokeStyle = `rgba(201,163,92,${0.05 + rnd() * 0.09})`;
      g.lineWidth = 1;
      g.beginPath(); g.moveTo(x, 120); g.lineTo(x + (rnd() - 0.5) * 8, 212); g.stroke();
    }
    for (let i = 0; i < 9; i++) {
    ...
    for (let i = 0; i < 3; i++) {          /* old wounds */
      ...
      g.lineTo(sx + 16 + rnd() * 26, sy + (rnd() - 0.5) * 18); g.stroke();
    }
  ...
  let accR = 201, accG = 163, accB = 92, accPulse = -1e9, accPulseReq = false;
  ...
  return {
    ...
    draw(g, x, y, R, clockMs, opts) {
      ...
      if (accPulseReq) { accPulse = clockMs; accPulseReq = false; }
      ...
      const bp = Math.max(0, 1 - (clockMs - accPulse) / 900);
    ...
    /* the bridge considers a world: its display takes the accent and pulses.
       (Keeps the setSkin name so every hover/beat call site stands.) */
    setSkin(slug) {
